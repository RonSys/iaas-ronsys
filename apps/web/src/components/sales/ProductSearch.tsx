 ...
 *
 * HU-F0-015: Muestra precios mayoristas y filtro por categoría
 * - Precio unitario + mayorista si aplica
 * - Filtro por categoría (combobox)
 * - Misma API que el POS existente
 *
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/accounting/kardex/products?${params.toString()}`,
        );
        if (res.ok) {
