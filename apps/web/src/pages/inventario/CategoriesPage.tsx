/**
 * CategoriesPage — Gestión de categorías/grupos de productos.
 *
 * HU-F0-015: CRUD de categorías de 1 nivel
 * - Crear, editar, eliminar categorías
 * - Validación: no eliminar si tiene productos asignados
 *
 * @module pages/inventario/CategoriesPage
 */
...
export function CategoriesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const body: Record<string, string> = { name: categoryName.trim() };
  ...
  if (error && categories.length === 0) {
    return (
      <div className="space-y-4">
        <h2 className="text-xl font-bold text-brand-text-primary">🏷️ Categorías de Productos</h2>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="p-10 text-center text-brand-text-secondary">
          ...
          <p className="text-sm mt-1">Creá categorías para organizar tus productos (ej: Fierros, Tuberías).</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-brand-text-secondary">
            {categories.length} categoría(s) · 1 nivel de jerarquía
          </p>
        </div>
        <button
          onClick={openCreate}
