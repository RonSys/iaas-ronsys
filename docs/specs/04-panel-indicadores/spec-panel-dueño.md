...
- **CA-M3 — Ticket por canal/turno**: bloque `avg_ticket_by`; `channel` (convención V1: dine_in incluye takeout) + `shift` (morning 06-11:59, afternoon 12-17:59, evening 18-23:59 — D-M1); huecos con `ticket: 0, orders: 0`. Sin anuladas.
  - **Observación implementación (2026-08-11)**: el helper `_shift_of` mapea `sale_time` nulo → **morning** (fallback defensivo; en la práctica `sale_time` está poblado en las ventas reales). Comportamiento documentado, sin test específico (caso límite de datos).
